...
export class Controller<Context = any> {
  ...
  async executeAction(
    ...
    try {
      ...
      if (registeredAction && typeof registeredAction.function === "function") {
        ...
        throw new Error(
          `Action ${actionName} does not have a valid function implementation`
        );
      }
    } catch (e) {
  ...
  async act(
    ...
    const actionData =
      typeof action.modelDump === "function"
  ...
  private registerBasicActions(): void {
    ...
    // Go to URL action
    this.registry.action("Navigate to URL in the current tab", GoToUrlAction)(
      this,
      "go_to_url",
      {
        value: async function (
    ...
    // Wait action
    this.registry.action("Wait for x seconds default 3", WaitAction)(
      this,
      "wait",
      {
    ...
    this.registry.action("Click element", ClickElementAction)(
      this,
      "click_element",
      {
        value: async function (
          ...
          if (session.cachedState) {
            if (session.cachedState.selectorMap) {
              // We have a cached selector map, but we'll use the current one
              // to ensure we're working with the most up-to-date state
              console.debug("Using current selector map instead of cached one");
            }
          }
          ...
          try {
            ...
            if (
              browser.context &&
              typeof browser.context.pages === "function"
            ) {
    ...
    this.registry.action(
      ...
      value: async function (
        ...
        }: { browser: ExtendedBrowserContext; hasSensitiveData?: boolean }
      ): Promise<ActionResult> {
        // Check if the index is defined
        if (params.index === undefined) {
    ...
    // Open tab action
    this.registry.action("Open url in new tab", OpenTabAction)(
      this,
      "open_tab",
      {
        value: async function (
    ...
    this.registry.action(
      ...
      value: async function (
        ...
        const amountText =
          amount !== undefined ? `${amount} pixels` : "one page";
        ...
        try {
          ...
          for (const frame of page.frames()) {
            try {
              ...
              // Continue to next frame if an error occurs in this one
              console.debug(`Error getting options in frame: ${e}`);
            }
          }
          ...
          const msg = `Dropdown options for element ${
            params.index
          }:\n${allOptions.join("\n")}`;
          console.info(msg);
    ...
    this.registry.action(
      ...
      value: async function (
        params: ExtractPageContentAction,
        {
          browser,
          pageExtractionLlm,
        }: { browser: BrowserContext; pageExtractionLlm?: any }
      ): Promise<ActionResult> {
        ...
        try {
          ...
          for (const frame of page.frames()) {
            try {
              ...
              console.error(
                `Frame ${frameIndex} attempt failed: ${String(frameError)}`
              );
