...
export class BrowserContextConfig {
  ...
  constructor(config: Partial<BrowserContextConfig> = {}) {
    ...
    this.browserWindowSize = config.browserWindowSize || {
      width: 1280,
      height: 1100,
    };
    ...
    this.waitForNetworkIdlePageLoadTime =
      config.waitForNetworkIdlePageLoadTime ?? 0.5;
...
export class BrowserContext {
  ...
  static _enhancedCssSelectorForElement(
    ...
    try {
      ...
      const cssSelector = BrowserContext._convertSimpleXpathToCssSelector(
        element.xpath
      );
  ...
  private static _convertSimpleXpathToCssSelector(
    ...
    for (const part of parts) {
      ...
      if (part.includes("[")) {
        ...
        const basePartEscaped = basePart.includes(":")
          ? basePart.replace(":", "\\:")
          : basePart;
  ...
  async saveCookies(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Saved ${cookies.length} cookies to ${this.config.cookiesFile}`
      );
    } catch (e: unknown) {
  ...
  async loadCookies(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Loaded ${cookies.length} cookies from ${this.config.cookiesFile}`
      );
    } catch (e: unknown) {
   ...
   */
  async _updateState(focusElement = -1): Promise<BrowserState> {
    try {
      // Get the current page
   ...
   */
  async _waitForPageAndFramesLoad(options?: {
    ...
    try {
      ...
      if (
        ...
        error.message.startsWith("URL not allowed:")
      ) {
  ...
  private async _waitForStableNetwork(): Promise<void> {
    ...
    try {
      ...
      const networkIdleTime =
        (this.config.waitForNetworkIdlePageLoadTime || 0.5) * 1000; // In milliseconds
      ...
      return new Promise<void>((resolve) => {
        const checkNetworkIdle = () => {
          ...
          if (
            ...
            now - lastActivity >= networkIdleTime
          ) {
            cleanup();
            resolve();
            return;
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          standardError.message.includes("URL not allowed")
        ) {
          throw standardError;
        }
        ...
        try {
          ...
          // If URL not allowed error, rethrow it
          if (
            jsClickErr instanceof Error &&
            jsClickErr.message.includes("URL not allowed")
          ) {
            throw jsClickErr;
          }
        ...
        try {
          ...
          if (elementHandle) {
            try {
              ...
              // Silently ignore scroll errors - this is intentional
              // Some elements cannot be scrolled into view but we still want to continue
            }
  ...
  async _inputTextElementNode(elementNode: any, text: string): Promise<void> {
    try {
      ...
      const isContentEditableHandle = await elementHandle.getProperty(
        "isContentEditable"
      );
      ...
      const isContentEditable =
        (await isContentEditableHandle.jsonValue()) as boolean;
      const readonly = readonlyHandle
        ? ((await readonlyHandle.jsonValue()) as boolean)
        : false;
      const disabled = disabledHandle
        ? ((await disabledHandle.jsonValue()) as boolean)
        : false;
  ...
  async reinitializePage(): Promise<void> {
    try {
      ...
      if (page) {
        await page
          .close()
          .catch((e: unknown) => console.warn("Error closing page:", e));
      }
  ...
  async closeCurrentTab(): Promise<void> {
    ...
    if (
      ...
      session.context.pages.length > 0
    ) {
  ...
  async _checkAndHandleNavigation(page: any): Promise<void> {
    ...
    if (!this._isUrlAllowed(url)) {
      ...
      try {
        ...
        console.error(
          `Failed to go back after detecting non-allowed URL: ${e}`
        );
      }
      throw new Error(`Navigation to non-allowed URL: ${url}`);
    }
