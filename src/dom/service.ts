...
export class DOMService {
  ...
  constructor(page: any) {
    ...
    try {
      ...
      this.jsCode =
        '() => ({ rootId: "root", map: { "root": { tagName: "DIV", children: [] } } });';
    }
  }
  ...
  async getClickableElements(
    ...
    focusElement = -1,
    viewportExpansion = 500 // Default matches Python implementation
  ): Promise<{
  ...
  private async buildDomTree(
    ...
    const viewportExpansionValue =
      viewportExpansion === undefined ? 500 : viewportExpansion;
  ...
  private async constructDomTree(
    ...
    if (!jsNodeMap || !jsRootId) {
      throw new Error(
        `Missing required DOM tree data: map=${!!jsNodeMap}, rootId=${!!jsRootId}`
      );
    }
  ...
  private parseNode(
    nodeData: any
  ): [DOMElementNode | DOMTextNode | null, string[]] {
    if (!nodeData) {
      return [null, []];
    }
